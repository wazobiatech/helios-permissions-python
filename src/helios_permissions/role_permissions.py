...
Canonical source of truth for the platform. The Helios service
(wazobiatech/helios) imports this module via the published package;
Helios does NOT redefine the map. TS / Python / Go / Laravel SDKs that
read the same JSON contract agree on what OWNER / ADMIN / EDITOR / VIEWER
means.
...
When a new service is onboarded:
  ...
  1. Add the perm strings to the appropriate ``*_PERMISSIONS`` tuple.
  2. Add the service-prefix to ``ROLE_PERMISSIONS`` for each role.
  3. ``mypy --strict`` will refuse to compile if any role is missing.
...
# Use ``Permission = Literal[...]`` style at type-check time via TYPE_CHECKING.
...
# The four roles. Mirrors ``RoleType`` in Helios's Prisma schema.
...
def is_permission(value: object) -> bool:
    ...
    if not isinstance(value, str):
        return False
    return (
        value in ATHENS_PERMISSIONS
        or value in MERCURY_PERMISSIONS
        or value in MUSE_PERMISSIONS
        or value in HELIOS_PERMISSIONS
    )
